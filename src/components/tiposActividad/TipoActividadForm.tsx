...
import type {
  TipoActividad,
  TipoActividadFormData,
  CreateTipoActividadDto,
  UpdateTipoActividadDto,
} from '../../types/tipoActividad.types';

interface TipoActividadFormProps {
  initialData?: TipoActividad | null;
  onSubmit: (data: CreateTipoActividadDto | UpdateTipoActividadDto) => void;
  onCancel: () => void;
  loading?: boolean;
  error?: string | null;
}
...
/**
 * Componente de formulario para crear/editar Tipos de Actividad
 */
export const TipoActividadForm: React.FC<TipoActividadFormProps> = ({
  initialData,
  onSubmit,
  onCancel,
  loading = false,
  ...
  const [formData, setFormData] = useState<TipoActividadFormData & { activo?: boolean }>({
    codigo: '',
    nombre: '',
    descripcion: '',
    orden: undefined,
  ...
  const handleSubmit = useCallback((e: React.FormEvent) => {
    ...
    const dataToSubmit: CreateTipoActividadDto | UpdateTipoActividadDto = {
    ...
    if (initialData) {
      (dataToSubmit as UpdateTipoActividadDto).activo = formData.activo;
    }
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={{ xs: 12, sm: 6 }}>
          <TextField
            ...
            helperText={validationErrors.codigo || 'Ejemplo: CORO, TALLER, CLASE_CANTO'}
            required
            disabled={loading}
        ...
        <Grid size={{ xs: 12 }}>
          <TextField
            ...
            helperText={validationErrors.nombre || 'Nombre descriptivo del tipo'}
            required
            disabled={loading}
          />
        </Grid>
        ...
        <Grid size={{ xs: 12 }}>
          <TextField
            ...
            helperText={validationErrors.descripcion || 'Descripción opcional del tipo (máx. 500 caracteres)'}
            multiline
            rows={3}
            disabled={loading}
          />
        ...
        {isEditing && (
          <Grid size={{ xs: 12 }}>
            <FormControlLabel
              ...
              label="Tipo de actividad activo"
            />
          </Grid>
        )}
